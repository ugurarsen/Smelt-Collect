...
namespace SupersonicWisdomSDK
{
    internal class SwFilesCacheManager
    {
        ...
        public SwFilesCacheManager(ISwLogger logger = null)
        {
            _logger = logger ?? SwInfra.Logger;
        }
        ...
        public void Clear ()
        {
            _logger.Log("Clear cached files");

            foreach (var file in GetAllFilesFromCache())
            {
                DeleteFile(file);
                _logger.Log("Removed cached file | " + file);
            }
        }
        ...
        public void DeleteFile(string filePath)
        {
            try
            {
                ...
                _logger.Log($"SwCacheManager | Deleted file | {filePath}");
            }
            catch (Exception e)
            {
                _logger.LogError("Deleting file failed " + e);
            }
        }
        ...
        public void WriteFile(string filePath, byte[] bytes)
        {
            try
            {
                ...
                _logger.Log($"SwCacheManager | WriteFile | {fileFullPath}");
            }
            catch (Exception e)
            {
                _logger.LogError(e.Message);
        ...
        private void CreateDirectories(string path, bool fromFilePath = false)
        {
            ...
            _logger.Log($"SwCacheManager | Created directory | {path}");
        }
