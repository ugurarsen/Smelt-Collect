...
namespace SupersonicWisdomSDK
{
    public class SwApi
    {
        ...
        public void Initialize(Dictionary<string, object> initParams = default)
        {
            ...
            {
                SwInfra.Logger.LogError($"Integration with SupersonicWisdomSDK is not complete. Fix the following issues: {string.Join(";", validationErrors)}");
                Application.Quit(1);
            }

            RunAllDeferredActions();

            SwContainerUtils.SetupContainer(Container, SwConstants.GameObjectName);
        }
        ...
        protected internal void AssertContainer(string fnName)
        {
            if (!_hasContainer)
            {
        ...
        internal void RunWithContainerOrThrow([NotNull] Action action, string fnName)
        {
            AssertContainer(fnName);
            action.Invoke();
        }
