    ...
    internal static class SwAttributionIosInjector
    {
        ...
        public static void OnPostProcessBuild(BuildTarget buildTarget, string path)
        {
            if (buildTarget == BuildTarget.iOS)
            {
                ...
                {
                    SwEditorLogger.LogError(
                        $"An error had occurred while adding SKAdNetworks to the Info.plist: {e.Message}");
                    SwEditorLogger.LogError(
                        $"The built XCode Project in {path} is invalid and the app will quit immediately.");
                }
                finally
                {
