...
public class Switch_Slide : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (SaveLoadManager.HasVibration())
        {
            Handle_Image_Off.color = newColor;
            Handle_Image_On.color = currentcolor2;
            Toggle_Pixel_Dimension.GetComponent<RectTransform>().anchoredPosition = End_Point;
            isoff = false;
            textIsOn = true;
        }
    }
